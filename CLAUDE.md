...
## Деплой после задач (ВАЖНО: прод и тест — одна ветка main, git push деплоит в оба сразу)

После выполнения КАЖДОЙ задачи:
1. Запусти: npx vitest run
2. Если все тесты зелёные → npm run build (проверка сборки) → graphify update . → git add -A → git commit
3. Проверь изменение вживую через `dev-test` (локально, тестовая БД) — там, где это применимо (UI-изменения)
4. **СТОП. Не пуш.** Дождись явного подтверждения пользователя ("ок", "пушь", "давай в прод" и т.п.)
5. Только после подтверждения — git push
6. Если есть упавшие тесты → сначала починить, потом повторить с шага 1
7. Сообщи результат: тесты / коммит / статус (запушено или ждёт подтверждения)
...
2. Если ВСЕ тесты зелёные → `graphify update .` → `git add -A` → `git commit -m "[описание на английском]"`
3. Проверь вживую через `dev-test` (тестовая БД), где применимо
4. **СТОП — не пушить самостоятельно.** Дождись явного "ок"/"пушь" от пользователя, только потом `git push` (прод и тест на одной ветке, пуш уходит сразу в оба)
5. Если есть упавшие тесты → НЕ коммитить/не пушить. Сначала починить, потом повторить шаг 1
6. Сообщи результат: количество тестов / что закоммичено / запушено или ждёт подтверждения
...
- Пушить без явного подтверждения пользователя (прод и тест на одной ветке — пуш деплоит сразу в оба)
...
- **Экспериментальные/непроверенные правки тоже фиксируй в TASKS.md** — статусом `IN_PROGRESS`, с пометкой, что именно не подтверждено (например "ждёт проверки на конкретном товаре/сценарии"). Не дожидайся полного подтверждения результата, чтобы записать хотя бы факт правки — иначе коммит в git есть, а контекст "зачем и сработало ли" нигде не сохранён и теряется безвозвратно (особенно в Telegram-сессиях — там весь контекст решения остаётся только в переписке Telegram, которая недоступна другим сессиям).
