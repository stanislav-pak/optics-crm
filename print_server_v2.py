...
def build_tspl(data: dict, quantity: int) -> bytes:
    ...
    # +2.5мм к смещению зазора — компенсация "просадки" после первых 2-3 этикеток
    # партии, которую уже некуда деть сдвигом текста внутри холста (штрихкод там
    # уже упирается в top_y=0, а GAP-offset двигает весь холст, а не текст в нём).
    cmd('GAP 7 mm,9.5 mm')
    cmd('DIRECTION 0')
    cmd('CODEPAGE 1251')
    cmd('CLS')
    ...
    # PRICE_SHIFT_X и LOWER_Y для штрихкода — разные для EAN-13 и CODE128 (у них разный
    ...
    PRICE_SHIFT_X_EAN13   = SHIFT_X - round(2 * DPI / 25.4)   # было -4мм, ещё +2мм от хвостика
    ...
    # там же, где сейчас хорошо — а штрихкод (он и так был прижат к 0 холста
    # независимо от этого числа) получит нужный толчок только от GAP-offset.
    RAISE_Y = round(1.5 * DPI / 25.4)
    ...
    if h_mm <= 12:
        ...
        if barcode:
            ...
            if len(barcode) == 13 and barcode.isdigit():
                ...
                top_y = max(0, (H - content_h) // 2 - RAISE_Y) + LOWER_Y_EAN13
                price_shift_x = PRICE_SHIFT_X_EAN13
                ...
                top_y = max(0, (H - content_h) // 2 - RAISE_Y) + LOWER_Y_CODE128
                price_shift_x = PRICE_SHIFT_X_CODE128
            ...
            if price_label:
                ...
                # Вертикаль цены жалоб не вызывала ни у одного типа — не меняется по типу.
                p_y = max(0, (H - ch_h) // 2 - RAISE_Y) + round(1 * DPI / 25.4)
