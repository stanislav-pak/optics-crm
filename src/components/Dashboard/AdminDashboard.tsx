      ...
      <div className="px-6 py-4 bg-[#202c33] border-b border-white/5">
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-4 text-xs text-[#8696a0]">
            ...
            {totalAmount > 0 && (
              <span>Сумма: <span className="text-emerald-400 font-medium">{totalAmount.toLocaleString('ru-RU')} ₸</span></span>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 max-w-xs">
            ...
            <input type="text" placeholder="Поиск по клиенту..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      <div className="flex-1 overflow-x-auto p-4">
        <div className="grid grid-cols-5 gap-3 h-full w-full">
          {STAGES.map((stage) => {
            ...
            return (
              <div key={stage.key} className="flex flex-col bg-[#202c33] rounded-xl overflow-hidden">
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2">
                  ...
                  {stageChats.map((chat) => (
                    <div key={chat.id} className={`rounded-lg transition-colors ${
