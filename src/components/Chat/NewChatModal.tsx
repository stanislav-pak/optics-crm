...
import { X, Search, MessageCircle, UserPlus, BookUser, ArrowLeft } from 'lucide-react';
...
export default function NewChatModal({ employee, onClose, onChatOpen }: NewChatModalProps) {
  ...
  // iOS: отдельный экран с type="tel" вверху и результатами снизу
  ...
  useEffect(() => {
    if (!iosMode) inputRef.current?.focus();
  }, [iosMode]);

  useEffect(() => {
    if (iosMode) setTimeout(() => telInputRef.current?.focus(), 100);
  }, [iosMode]);
  ...
  // Android: Contact Picker API — одно нажатие, нативный список
  const handlePickContact = async () => {
    try {
      ...
      if (phone) { setSearch(phone); setContactName(name); }
    } catch {
      // отменено
    }
  };

  // iOS: пользователь набирает / выбирает через клавиатуру
  // Инпут НЕ должен размонтироваться пока открыты нативные контакты iOS
  ...
  // ─── iOS режим: отдельный экран ─────────────────────────────────────────────
  // tel-инпут ВВЕРХУ → результаты СРАЗУ ПОД НИМ → ничего не надо скроллить
  if (iosMode) {
    return (
      <div data-modal="true" className="fixed inset-0 z-50 flex flex-col bg-[#0b141a]">
        <div className="flex items-center gap-3 px-4 py-3 bg-[#202c33] border-b border-white/10 flex-shrink-0">
          ...
          <span className="text-white font-semibold text-base">Выбрать контакт</span>
        </div>
        ...
        <div className="px-3 py-3 bg-[#111b21] border-b border-white/5 flex-shrink-0">
          ...
          <p className="text-[10px] text-[#8696a0] mt-2 px-1">
            Нажмите на иконку книжки в клавиатуре чтобы выбрать из контактов
          </p>
        </div>
          ...
          {search.trim().length < 2 && (
            <div className="flex flex-col items-center justify-center h-40 px-6">
              <p className="text-sm text-[#8696a0] text-center">
                Начните вводить номер — совпадения появятся здесь
              </p>
            </div>
          )}
        </div>
    ...
    <div data-modal="true" className="fixed inset-0 z-50 flex flex-col bg-[#0b141a]">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {search.trim().length < 2 && (
          <div className="flex flex-col items-center justify-center h-40 px-6">
            <p className="text-sm text-[#8696a0] text-center">
              Введи имя или номер телефона, или выбери из контактов
            </p>
          </div>
        )}
      </div>
