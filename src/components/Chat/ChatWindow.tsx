  ...
  return (
    <div className="flex h-full overflow-hidden">
      <div className="flex flex-col flex-1 min-w-0">
        ...
        {/* Input — WhatsApp style single row */}
        <div className="px-2 py-2 bg-[#202c33] flex-shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative flex-shrink-0">
              <button onClick={(e) => { e.stopPropagation(); setShowAttachMenu(v => !v); }} disabled={isArchived}
                className="w-9 h-9 text-[#8696a0] hover:text-[#e9edef] disabled:opacity-50 flex items-center justify-center transition-colors">
                    ...
                    <button onClick={async () => {
                      ...
                      if ('contacts' in navigator) {
                        try {
                          ...
                          if (contacts?.length > 0) { setContactName(contacts[0].name?.[0] || ''); setContactPhone(contacts[0].tel?.[0] || ''); }
                        } catch {}
                      }
            ...
            {/* Центр: инпут или запись */}
            <div className="flex-1 bg-[#2a3942] rounded-full flex items-center min-h-[42px] overflow-hidden">
              {isRecording ? (
                /* Запись внутри инпута */
                <div className="flex items-center w-full px-3 gap-2">
                  <button onClick={() => stopRecording(true)} className="text-[#8696a0] hover:text-red-400 text-lg leading-none flex-shrink-0">✕</button>
                  <span className="flex-1 text-red-400 text-sm font-mono animate-pulse">⏺ {formatRecTime(recordingTime)}</span>
                  <button onPointerUp={() => stopRecording()}
                    className="w-7 h-7 bg-red-500 rounded-full flex items-center justify-center flex-shrink-0 my-1.5">
                    <svg className="w-3.5 h-3.5 text-white" fill="currentColor" viewBox="0 0 24 24"><rect x="6" y="6" width="12" height="12" rx="2"/></svg>
                  </button>
                </div>
              ) : (
                <textarea value={text} onChange={(e) => setText(e.target.value)} onKeyDown={handleKeyDown}
                  ...
                  className="flex-1 bg-transparent text-[#d1d7db] placeholder-[#8696a0] px-4 py-2.5 text-sm outline-none resize-none max-h-32 w-full disabled:opacity-50 disabled:cursor-not-allowed"
