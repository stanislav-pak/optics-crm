...
export default function ReturnModal({ sales, employeeId, onClose, onSuccess, initialSaleId }: Props) {
  ...
  // Сколько клиент реально заплатил за мастерскую (#6: explicit null check, not ??)
  const basePrepay = workshopOrder
    ? (workshopOrder.original_prepayment !== null && workshopOrder.original_prepayment !== undefined
        ? workshopOrder.original_prepayment
        : (workshopOrder.prepayment ?? 0))
    : 0;
  const workshopPaidAmount = workshopOrder
    ? basePrepay + (workshopOrder.remaining_paid_at
        ? (workshopOrder.service_price + workshopOrder.parts_price) - basePrepay
        : 0)
    : 0;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (returnWorkshop && workshopOrder) {
        ...
        const { error: workshopError } = await supabase
          ...
          .update({
            ...
            prepayment_refunded_at: workshopOrder.original_prepayment > 0 ? now : null,
      ...
      window.dispatchEvent(new CustomEvent('sale-returned', { detail: { saleId: selectedSale?.id } }));
  ...
  return (
    ...
    >
      <div className="w-full sm:max-w-lg rounded-t-3xl flex flex-col bg-white" style={{ maxHeight: '85vh' }}>
        {/* Drag handle */}
        ...
        <div className="flex items-center justify-between px-5 py-3 flex-shrink-0">
          <div className="flex items-center gap-2">
            {step === 'items' && !initialSaleId && (
              <button onClick={() => setStep('select')} className="p-1 rounded-full mr-1 text-gray-500">
                <ChevronLeft size={20} />
              </button>
            )}
          ...
          <button onClick={onClose} className="p-1 rounded-full text-gray-500">
            <X size={20} />
          </button>
        </div>
            ...
            <div className="px-5 pb-3 flex-shrink-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl pl-9 pr-4 py-2.5 text-sm focus:outline-none bg-gray-50 border border-gray-200 text-gray-900"
                />
              </div>
            </div>
                  ...
                  {filteredSales.map(s => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left rounded-2xl px-4 py-3 transition-colors active:opacity-80 bg-gray-50 border border-gray-200"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            <p className="text-sm font-medium truncate text-gray-900">{clientName}</p>
                          ...
                          <div className="text-right flex-shrink-0">
                            <p className="text-sm font-semibold text-gray-900">₸{s.total.toLocaleString()}</p>
        ...
        {step === 'items' && selectedSale && (
          <>
            {/* Инфо о продаже */}
            <div className="mx-5 mb-3 rounded-2xl px-4 py-3 flex-shrink-0 bg-gray-50">
            ...
            >
              ...
              {/* Позиции товаров */}
              {items.length > 0 && (
                <div className="rounded-2xl overflow-hidden divide-y bg-gray-50 border border-gray-200">
                  {items.map(item => {
                    ...
                    return (
                      <div key={item.product_id} className="px-4 py-3">
                        <div className="flex items-start justify-between gap-3 mb-2">
                          <div className="min-w-0">
                            <p className="text-sm font-medium truncate text-gray-900">{productName}</p>
                            <p className="text-xs mt-0.5 text-gray-500">
                        ...
                        <div className="flex items-center rounded-xl overflow-hidden bg-white border border-gray-200">
                          <button
                            type="button"
                            onMouseDown={e => { e.preventDefault(); setQtys(prev => ({ ...prev, [item.product_id]: Math.max(0, (prev[item.product_id] ?? 0) - 1) })); }}
                            className="px-4 py-2 text-lg font-medium flex-shrink-0 text-gray-500"
                          >−</button>
                          <input
                            type="text"
                            inputMode="numeric"
              ...
              {workshopHasPayment && workshopOrder && workshopOrder.status !== 'cancelled' && (
                <div>
                  <p className="text-xs font-medium mb-2 text-gray-500">Услуги мастерской</p>
                  <div className="rounded-2xl px-4 py-3 bg-gray-50 border border-gray-200">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-gray-900">🔧 {workshopOrder.service_name}</p>
                    ...
                    {returnWorkshop && (
                      <p className="text-xs mt-2 font-medium text-blue-500">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full rounded-xl px-4 py-3 text-sm resize-none focus:outline-none bg-gray-50 border border-gray-200 text-gray-900"
                />
              </div>
              ...
              {(totalReturnQty > 0 || returnWorkshop) && reason.trim().length > 0 && (
                ...
                >
                  ...
                  <p className="text-xs text-gray-500">Причина: {reason.trim()}</p>
                </div>
              )}
