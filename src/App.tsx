    ...
    <div className={`${isMobile ? 'w-full' : 'w-80 flex-shrink-0 border-r border-white/5'} flex flex-col`}>
      <div className="px-4 py-3 bg-[#202c33] flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {employee.role === 'admin' && (
            <>
              <button onClick={() => { setAdminView('dashboard'); setActiveChat(null); if (isMobile) setMobileView('list'); }}
              ...
              <button onClick={() => { setAdminView('reports'); setActiveChat(null); if (isMobile) setMobileView('list'); }}
                className={`px-2 py-1 rounded-lg transition-colors ${adminView === 'reports' ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`} title="Аналитика">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
              </button>
              <button onClick={() => { setAdminView('activity'); setActiveChat(null); if (isMobile) setMobileView('list'); }}
                className={`px-2 py-1 rounded-lg transition-colors ${adminView === 'activity' ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`} title="Активность">
