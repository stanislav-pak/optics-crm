...
function AppContent() {
  ...
  const Sidebar = (
    <div className={`${isMobile ? 'w-full' : 'w-80 flex-shrink-0 border-r border-white/5'} flex flex-col`}>
      ...
      {isAdmin && isMobile && (
        <div className="flex bg-[#202c33] border-t border-white/10 flex-shrink-0">
          <button onClick={() => { setAdminView('inventory'); setMobileView('main'); }}
            className={`flex-1 py-2.5 flex flex-col items-center gap-0.5 transition-colors ${adminView === 'inventory' ? 'text-emerald-400' : 'text-[#8696a0]'}`}>
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
            <span className="text-[10px] font-medium">Склад</span>
          </button>
        </div>
