          ...
          {isAdmin && (
            <>
              <button onClick={() => { setAdminView('dashboard'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('dashboard') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
              ...
              <button onClick={() => { setAdminView('tasks'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('tasks') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
                title="Задачи">
              ...
              <button onClick={() => { setAdminView('reports'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('reports') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
                title="Аналитика">
              ...
              <button onClick={() => { setAdminView('activity'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('activity') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
                title="Активность">
              ...
              <button onClick={() => { setAdminView('inventory'); setActiveChat(null); if (isMobile) setMobileView('inventory'); }}
                className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('inventory') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
                title="Склад">
              ...
              <button onClick={() => setShowImport(true)}
                className="px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 text-[#8696a0] hover:text-[#e9edef]" title="Импорт Excel">
                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
              </button>
              {employee.role === 'admin' && (
                <button
                  onClick={() => { setAdminView('settings'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                  className={`px-1.5 py-1 rounded-lg transition-colors flex-shrink-0 ${isAdminBtnActive('settings') ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`}
                  title="Настройки">
                  <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
