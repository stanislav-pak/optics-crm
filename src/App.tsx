      ...
      <div className="px-4 py-3 bg-[#202c33] flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-[#8696a0]">{employee.role === 'admin' ? 'РђРґРјРёРЅРёСЃС‚СЂР°С‚РѕСЂ' : 'РњРµРЅРµРґР¶РµСЂ'}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {employee.role === 'admin' && (
            <>
              <button onClick={() => { setAdminView('dashboard'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-2 py-1 rounded-lg transition-colors ${adminView === 'dashboard' && !activeChat ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`} title="Dashboard">
              ...
              <button onClick={() => { setAdminView('reports'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-2 py-1 rounded-lg transition-colors ${adminView === 'reports' ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`} title="РђРЅР°Р»РёС‚РёРєР°">
              ...
              <button onClick={() => { setAdminView('activity'); setActiveChat(null); if (isMobile) setMobileView('main'); }}
                className={`px-2 py-1 rounded-lg transition-colors ${adminView === 'activity' ? 'bg-emerald-500 text-white' : 'text-[#8696a0] hover:text-[#e9edef]'}`} title="РђРєС‚РёРІРЅРѕСЃС‚СЊ">
          ...
          {employee.role === 'admin' && (
              <button onClick={() => setShowImport(true)}
                className="px-2 py-1 rounded-lg transition-colors text-[#8696a0] hover:text-[#e9edef]" title="РРјРїРѕСЂС‚ Excel">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4" /></svg>
              </button>
            )}
          <button onClick={() => signOut()} className="text-[#8696a0] hover:text-[#e9edef] transition-colors" title="Р’С‹Р№С‚Рё">
      ...
      {employee.role === 'admin' && adminView === 'reports' ? (
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-[#8696a0] text-sm">Р’С‹Р±РµСЂРё С‡Р°С‚ С‡С‚РѕР±С‹ РЅР°С‡Р°С‚СЊ</p>
          </div>
        </div>
      )}
    </div>
