...
> **Следующая задача: T45 — Баги в заявках на склад (T39)** (см. ниже, есть готовый SQL-фикс, ждёт подтверждения)
...
### T45 — Баги в заявках на склад (продолжение T39) `TODO` (2026-07-04)
**Контекст:** при реальном тестировании T39 (двухэтапный запрос товара со склада) нашлись баги.

**1. Кнопка "Отправить" не появляется после "Одобрить" — причина найдена, нужен фикс в БД.**
RLS-политика `admin_update_requests` на таблице `stock_requests` разрешает UPDATE только ролям `admin`/`branch_admin`. Реальный сотрудник склада ("Aidana cklad") имеет роль `manager` — его "Одобрить" выполняется без ошибки, но БД молча не применяет изменение (0 строк обновлено, RLS не кидает исключение). Статус так и остаётся `new`, поэтому кнопка "Отправить" (видна только при `status==='approved'`) никогда не показывается. Подтверждено: все 3 тестовые заявки в базе всё ещё `new`.

Готовый SQL (ждёт подтверждения пользователя — DB-изменение):
```sql
drop policy "admin_update_requests" on stock_requests;

create policy "admin_update_requests" on stock_requests for update using (
  get_user_role() = ANY (ARRAY['admin','branch_admin'])
  or exists (select 1 from employees where user_id = auth.uid() and branch_id = 'a215f402-07ee-4ba9-aba5-b2b4cd5497f2')
);
```
(WAREHOUSE_ID = `a215f402-07ee-4ba9-aba5-b2b4cd5497f2`, см. `src/constants.ts`)

**2. Push-уведомления не доходят — частично диагностировано, не завершено.**
- У Дианары (филиал "Гум", создатель заявок) в `push_subscriptions` вообще нет записи — физически некуда слать, пока она не откроет приложение и не разрешит уведомления заново.
- У складского сотрудника ("Aidana cklad") подписка есть. Прямой тестовый вызов `send-push` edge-функции на её `employee_id` вернул `{"success":true,"sent":1}` — но это не гарантирует реальную доставку (функция не проверяет результат `webPush.sendNotification`, только ловит 410/404 для очистки мёртвых подписок, остальные ошибки просто уходят в `console.error` и не видны в HTTP-ответе).
- **Нужно уточнить у пользователя:** дошло ли тестовое уведомление "Тест диагностики / Проверка push от Claude" до Айданы. Если нет — копать дальше (VAPID-ключи edge-функции, expired subscription и т.д.). Если да — значит push технически работает, и проблема была только в п.1 (RLS блокировал апрув → но notifyBranch должен был всё равно вызываться после UPDATE, т.к. `updErr` не кидался — это тоже стоит перепроверить на живых данных после фикса RLS).

**3. Поле количества в заявке — фикс закоммичен (`ca295cd`), пользователь ещё не подтвердил результат.**
Просил проверить в приватном/анонимном окне браузера (на случай кэша) — ответа пока нет.

**Файлы:** `src/services/inventory.ts` (approveStockRequest/shipStockRequest/notifyBranch), `src/pages/InventoryPage.tsx`, миграция БД (не применена).
...
### T46 — Индикаторы непрочитанного (бейдж на иконке PWA + точки в списках) `TODO` (2026-07-05)
**Описание:** Найдено при тестировании push-уведомлений в тестовом окружении — уведомление приходит, но нигде в интерфейсе нет визуального признака "непрочитано": ни на иконке PWA (badge count), ни точками в списках (чаты, заявки и т.п.), как в WhatsApp.

**Логика (набросок, требует уточнения пользователем перед реализацией):**
- Точка/бейдж появляется когда пришло новое непрочитанное (push или новая запись)
- Пропадает **только** при реальном прочтении/открытии конкретного элемента — не при сворачивании приложения, не при простом получении уведомления, не при заходе в общий список без открытия самого элемента
- Нужно решить: это только про чаты, или также про заявки на склад, задачи и т.д.
- Нужно решить: badge на иконке PWA — общий счётчик по всем типам непрочитанного, или отдельно по каждому

**Важно:** пользователь ещё не определился с точной логикой — не начинать реализацию до уточнения.
